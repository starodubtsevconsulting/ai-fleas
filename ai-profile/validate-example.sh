...
required=(
  ...
  "example/commands-config/hermes-app/config.yml"
...
grep -Fq '    config: commands-config/hermes-app/config.yml' "${PROFILE}" || fail "Example must reference sanitized Hermes configuration"
grep -Fq '  root: ${AI_COMMANDS_ROOT}' "${ROOT}/example/commands-config/hermes-app/config.yml" || fail "Hermes example must use the resolved command root"
grep -Fq 'schema_version: local-ai-providers.v1' "${ROOT}/example/commands-config/hermes-app/config.yml" || fail "Hermes example must declare the provider-map schema"
grep -Fq '      url: http://127.0.0.1:8000/v1' "${ROOT}/example/commands-config/hermes-app/config.yml" || fail "Hermes example must use the safe loopback endpoint"
grep -Fq '        hermes:' "${ROOT}/example/commands-config/hermes-app/config.yml" || fail "Hermes example must map per-model Hermes settings"
grep -Fq 'EXAMPLE ONLY:' "${ROOT}/example/commands-config/hermes-app/config.yml" || fail "Hermes config must be labeled as example-only"
