...
The sanitized human example in `humans/jonathan-example/profile.yml` explicitly lists the profiles its Governor may inspect
and the profile-qualified workflows it may use. These authorizations belong to the human profile rather than the reusable
Governor role or its runtime binding. Profile authorization and workflow authorization are independent: access to a profile
does not implicitly grant access to all workflows in that profile. When an authorized work profile adds or removes a
workflow, update the human allowlist deliberately and run the example validator.
