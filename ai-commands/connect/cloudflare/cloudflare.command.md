...
default, and has a draggable horizontal divider so the provider list and logs can be resized independently. Filter
buttons show all connector output or only the log stream for one model-provider tunnel.
