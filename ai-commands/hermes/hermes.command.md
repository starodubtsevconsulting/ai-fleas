...
2. Create the Dev bot. The work profile supplies its default workflow, sole project, provider, endpoint, and model:
   ...
   This creates `example-dev-example-services`. The fully explicit equivalent is useful in automation or when a profile exposes
   multiple choices:
   ...
   The command resolves the workspace, endpoint, and provider model from `ai-profile/example-profile`, validates `/models` before
   mutation, creates `~/.hermes/profiles/example-dev-example-services`, creates the
   `~/.local/bin/example-dev-example-services` alias, configures the provider and local terminal, and writes the scoped SOUL file.
...
3. Create the Accounting bot from the same `sc` profile but a different workflow/project scope:
   ...
   ai-commands/hermes/hermes.command.sh setup \
     --work-profile sc \
     --workflow accounting \
     --project example-business
   ```

   This creates `example-accounting-example-business` with its terminal rooted at the Accounting project's configured
   `repo_path`.

4. Verify both profiles and their live model routes:

   ```bash
   ai-commands/hermes/hermes.command.sh show example-dev-example-services
   ai-commands/hermes/hermes.command.sh status example-dev-example-services
   ai-commands/hermes/hermes.command.sh show example-accounting-example-business
   ai-commands/hermes/hermes.command.sh status example-accounting-example-business
   ```
...
```bash
ai-commands/hermes/hermes.command.sh delete example-dev-example-services --confirm-delete
```
