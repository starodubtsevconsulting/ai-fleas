...
assert.match(contract, /Readiness without\s+catalog and sidebar presence is an explicit partial-initialization failure/);
assert.match(contract, /selected profile's canonical absolute directory and\s+exact resolved binding-registry path/);
assert.match(contract, /Normalize the saved project's sidebar order to the exact active roster/);
...
assert.match(contract, /reread the project catalog and the expanded saved-project sidebar/);
...
assert.match(initializationAdapter, /normalize the saved project's sidebar task order to the exact active roster/);
assert.match(initializationAdapter, /backend catalog but absent from the\s+expanded saved-project sidebar is still incomplete initialization/);
