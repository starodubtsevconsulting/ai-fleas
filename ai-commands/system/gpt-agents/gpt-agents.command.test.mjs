...
const governorInitialization = fs.readFileSync(path.join(root, 'platforms/gpt-agents/agents/personal-governor-initialization.md'), 'utf8');
const commandSpec = fs.readFileSync(path.join(commandDir, 'spec.md'), 'utf8');
...
assert.match(contract, /reread the project catalog, and inspect the rendered expanded\s+saved-project sidebar/);
...
assert.match(contract, /projectless task with the host's fresh-history new-task operation/);
assert.match(contract, /Never place the successor inside a workflow saved project or use a task-fork/);
assert.match(contract, /Conversation history is not Governor memory/);
assert.match(contract, /must not copy, summarize, reconstruct, or replay the predecessor transcript/);
assert.match(governorInitialization, /fresh-history, projectless task created through the platform's new-task primitive/);
assert.match(governorInitialization, /Never create it inside a workflow saved project or by forking, cloning/);
assert.match(governorInitialization, /Conversation history is not Governor memory/);
assert.match(commandSpec, /always a projectless, fresh-history task created with the platform's new-task primitive/);
assert.match(commandSpec, /not predecessor transcripts, summaries, turns, or reconstructed conversation context/);
