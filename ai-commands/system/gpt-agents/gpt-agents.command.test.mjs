...
assert.match(contract, /fresh-history new-task operation/);
assert.match(contract, /Never use a task-fork, clone-with-history, or equivalent inherited-context operation/);
...
assert.match(governorInitialization, /fresh-history task created through the platform's new-task primitive/);
assert.match(governorInitialization, /Never create it by forking, cloning, or otherwise inheriting the predecessor task/);
assert.match(governorInitialization, /Conversation history is not Governor memory/);
assert.match(commandSpec, /always a fresh-history task created with the platform's new-task primitive/);
