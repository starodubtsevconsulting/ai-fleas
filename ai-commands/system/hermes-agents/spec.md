...
- The model-provider startup source is not an agent-facing secret lookup. For work that needs another credential, a workflow Agent uses a selected, profile-authorized AI Fleas consumer through `secrets run <consumer> -- <operation>`. Generated workflow guidance must direct the Agent to this path when both commands are selected and must never direct it to print a value. No additional Hermes plugin is required for this existing command path; a new agent-facing integration requires a separately specified bounded consumer.
- When the selected workflow includes `secrets`, `initialize` and `reconcile` validate that profile's secrets command configuration before changing any Hermes profiles. This check is offline and must not claim provider authentication, secret existence, or consumer success. A protected model route additionally uses the `hermes-agents` consumer at initialization and the bot's built-in startup source at backend launch.
...
- `initialize-system` realizes exactly one globally pinned profile with no workflow-group membership and exactly one profile-scoped scheduler. An optional `--connection` selects a named route from the System provider, including protected remote routes whose header secrets are resolved from environment variables. By default, its ordered watch set contains every Hermes workflow declared by the selected work profile. Explicit `--watch-group` arguments may select a narrower subset of that derived set but cannot cross the work-profile boundary. It writes or replaces the System receipt only after the profile, scheduler, gateway, and ticker are verified ready.
- A canonical System profile's runtime ID and visible title are `<profile-id>-system` (for example, `example-system`). Explicit test instances use `<profile-id>-system-<instance>` (for example, `example-system-2`) and have separate schedulers and receipts; they never replace the canonical System receipt.
