...
# datadog.sh
# Usage:
#   ${AI_COMMANDS_ROOT}/datadog/datadog.sh --service <service> [--env dev] [--range 15m|1h|1d|1w] [--mode logs|livetail] [--errors-only] [--print-only]

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
source "$SCRIPT_DIR/../command-python.setup.sh"
CONF_FILE="${DATADOG_COMMAND_CONF:-${AI_COMMAND_CONFIG_PATH:-}}"
