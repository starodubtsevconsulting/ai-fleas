...
service_probe="$(ssh "${ssh_args[@]}" "$target" 'set -eu; if systemctl is-active --quiet ai-local-provider 2>/dev/null; then echo service_ai_local_provider=active; elif systemctl cat ai-local-provider >/dev/null 2>&1; then echo service_ai_local_provider=inactive; else echo service_ai_local_provider=absent; fi; if systemctl is-active --quiet ollama 2>/dev/null; then echo service_ollama=active; elif systemctl cat ollama >/dev/null 2>&1; then echo service_ollama=inactive; else echo service_ollama=absent; fi; if systemctl --user is-active --quiet ai-policy-evaluator.service 2>/dev/null; then echo service_ai_policy_evaluator=active; elif systemctl --user cat ai-policy-evaluator.service >/dev/null 2>&1; then echo service_ai_policy_evaluator=inactive; else echo service_ai_policy_evaluator=absent; fi')"
probe="$probe"$'\n'"$service_probe"
if [[ "$action" == inspect ]]; then
