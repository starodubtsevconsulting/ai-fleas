...
`system`), service name, optional localhost-only health URL and bounded readiness timeout. An optional `verification`
object configures a public HTTPS boundary check (including accepted 2xx/3xx statuses) and a real localhost generation
request with its timeout and required non-empty JSON response path. `model-status` reports every
...
restart, and otherwise stops all configured modes before starting and checking local health, the configured public
endpoint, and real inference with periodic progress.
