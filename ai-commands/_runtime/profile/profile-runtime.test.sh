...
grep -Fq "AI_AGENT_RUNTIME_PROJECT_ROOT=$HOME/projects/example-workspace/example-service" <<< "$base"
grep -Fq 'AI_PRIMARY_PROJECT_ID=example-service' <<< "$base"
grep -Fq '/ai-profile/example/projects-config/dev/example-service/project.yml' <<< "$base"
grep -Fq "AI_PRIMARY_PROJECT_ROOT=$HOME/projects/example-workspace/example-service" <<< "$base"
