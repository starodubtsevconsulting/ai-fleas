...
| Complete and verify | In the scheduling state, find **Schedule to publish**. Then return to the story state or **Stories → Scheduled**. | Select the final scheduling control once when the configured review and acceptance policy, target, and slot checks pass. Verify the exact story, scheduled status, and date/time on Medium; preserve the URL and archive record. If the result is unclear, inspect the Scheduled list before retrying. |
...
scheduled time, so compare the reviewed revision again before pressing the final scheduling control.
