...
node -e '
  const result = require(process.argv[1]);
  if (result.ticket.cloneFrom !== "EXAMPLE-1234") throw new Error("clone source was not normalized");
...
if "$SCRIPT_DIR/jira.command.sh" clone EXAMPLE-1234 \
...
node -e '
  const result = require(process.argv[1]);
  if (result.ticket.updateIssue !== "EXAMPLE-1234") throw new Error("update issue was not normalized");
...
"$SCRIPT_DIR/jira.command.sh" set-in-progress EXAMPLE-1234 \
  --output-dir "$TEST_DIR/set-in-progress" \
  --validate-only >/dev/null
node -e '
  const result = require(process.argv[1]);
  if (result.status !== "validated") throw new Error("expected set-in-progress validation");
  if (result.issueKey !== "EXAMPLE-1234") throw new Error("issue key was not normalized");
...
"$SCRIPT_DIR/jira.command.sh" delete-owned-comment EXAMPLE-1234 \
  --comment-id 12345678 \
...
node -e '
  ...
  if (result.issueKey !== "EXAMPLE-1234") throw new Error("issue key was not inferred from the plan");
...
node -e '
  ...
  if (result.issueKey !== "EXAMPLE-1234") throw new Error("comment issue was not normalized");
...
if "$SCRIPT_DIR/jira.command.sh" comment EXAMPLE-1234 --body test --link "unsafe|http://example.com" --validate-only >/dev/null 2>&1; then
...
if "$SCRIPT_DIR/jira.command.sh" --delete EXAMPLE-1234 >/dev/null 2>&1; then
