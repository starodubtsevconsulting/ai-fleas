...
CONF_FILE="$SCRIPT_DIR/jira.command.conf"
...
if [[ -z "$AI_FLOW_OUTPUT_DIR" ]]; then
  AI_FLOW_OUTPUT_DIR="${TMPDIR:-/tmp}/ai-config-jira-sync"
fi
...
exec node "$SCRIPT_DIR/jira-smart-checklist.mjs" --output-dir "$AI_FLOW_OUTPUT_DIR" "${FORWARD_ARGS[@]}"
