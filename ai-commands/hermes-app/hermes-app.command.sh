...
resolve_hermes() {
  if [[ -n "${HERMES_BIN:-}" && -x "${HERMES_BIN}" ]]; then printf '%s\n' "${HERMES_BIN}";
  elif command -v hermes >/dev/null 2>&1; then command -v hermes;
  elif [[ -x "${HOME}/.local/bin/hermes" ]]; then printf '%s\n' "${HOME}/.local/bin/hermes";
...
case "${action}" in
  ...
  initialize|reconcile|configure|setup)
    ...
    [[ -z "${agent_instructions}" ]] || resolved_agent_instructions="${agent_instructions}"
    [[ -z "${instance}" ]] || validate_profile "${instance}"
    ...
    for role_binding in "${role_bindings[@]}"; do
      ...
      case "${role}" in admin) role_title='Admin';; designer-reviewer) role_title='Designer/Reviewer';; judge) role_title='Judge';; manager) role_title='Manager';; coder) role_title='Coder';; command-runner) role_title='Command Runner';; ui-acceptance-tester) role_title='UI Acceptance Tester';; *) role_title="${role}";; esac
    ...
    ;;
  list) [[ $# -eq 0 ]] || { usage >&2; exit 2; }; "$(resolve_hermes)" profile list ;;
  show) [[ $# -eq 1 ]] || { usage >&2; exit 2; }; validate_profile "$1"; "$(resolve_hermes)" profile show "$1" ;;
  status)
    [[ $# -eq 1 ]] || { usage >&2; exit 2; }; profile="$1"; validate_profile "${profile}"; hermes_bin="$(resolve_hermes)"
    provider="$("${hermes_bin}" -p "${profile}" config get model.provider)"; model="$("${hermes_bin}" -p "${profile}" config get model.default)"; endpoint="$("${hermes_bin}" -p "${profile}" config get model.base_url)"; workspace="$("${hermes_bin}" -p "${profile}" config get terminal.cwd)"
    printf 'HERMES_READY\nProfile: %s\nProvider: %s\nModel: %s\nEndpoint: %s\nWorkspace: %s\n' "${profile}" "${provider}" "${model}" "${endpoint%/}" "${workspace}"
    ;;
  delete) [[ $# -eq 2 && "$2" == '--confirm-delete' ]] || { printf '%s\n' 'HERMES_DELETE_CONFIRMATION_REQUIRED: use delete PROFILE --confirm-delete.' >&2; exit 2; }; profile="$1"; validate_profile "${profile}"; "$(resolve_hermes)" profile delete "${profile}" --yes; printf 'HERMES_PROFILE_DELETED: %s\n' "${profile}" ;;
  *) usage >&2; exit 2 ;;
esac
