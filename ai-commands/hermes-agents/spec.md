...
- `initialize-system` realizes exactly one globally pinned profile with no workflow-group membership and exactly one profile-scoped scheduler covering its ordered watch set. It writes or replaces the System receipt only after the profile, scheduler, gateway, and ticker are verified ready.
