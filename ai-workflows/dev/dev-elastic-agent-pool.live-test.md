...
disposable logical project such as `example-dev-elastic-test`; never run it in base `example-dev`.
...
tickets, deployments, commands, credentials, and every task in base `example-dev` are out of scope. Record the Admin,
