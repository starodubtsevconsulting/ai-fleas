...
```mermaid
flowchart TD
  Actor["Actor: Human authorizes the elastic-pool live test"] --> Scope{"Decision: explicit isolated example-dev instance selected?"}
  Scope -->|Yes| Run["Allowed: use zero-write token assignments and verify scale-up/settlement"]
  Scope -->|No| Blocked["BLOCKED: never test destructive lifecycle behavior in base example-dev"]
...
It requires direct human authorization, an explicit `example-dev-<instance-id>` logical project, zero product/repository/
tracker/external effects, and complete cleanup evidence. The base `example-dev` project is prohibited for this scenario.
