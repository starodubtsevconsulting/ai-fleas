...
## Qwen-Image BF16 early evidence

Qwen-Image was evaluated at pinned revision `75e0b4be04f60ec59a75f475837eced720f823b6` under Apache 2.0. The task was stopped early by operator decision. The retained evidence is therefore exactly 12 images: four Animation Workflow anime cases, three fixed seeds per case. It is intentionally reported as early evidence rather than a completed comparative benchmark.

The sanitized machine-readable record is [`results/qwen-image-gx10-2026-09-22.json`](results/qwen-image-gx10-2026-09-22.json). Retained PNGs remain in private benchmark storage. Later runner artifacts are excluded from the retained set and from all result claims.

| Measurement | Observed result |
|---|---:|
| Retained scope | 12 images; four cases × three seeds |
| Cases | 16:9 story frame, portrait, full body, indoor frame |
| Seeds | 20260922, 20260923, 20260924 |
| Precision/runtime | BF16, `DiffusionPipeline.from_pretrained`, CUDA ARM64 container |
| Model load | 349.71 s |
| Generation latency | 153.69–155.67 s |
| Median / mean generation | 154.03 s / 154.44 s |
| Steps / guidance | 50 / `true_cfg_scale=4.0` |
| Peak whole-system used during retained generations | 72.45 GB |
| Minimum whole-system available during retained generations | 58.14 GB |
| Peak PyTorch allocated / reserved | 62.18 GB / 65.20 GB |
| Retained output bytes | 10,535,487 |
| Validation | 12/12 hashes, byte counts, dimensions, and RGB PNG decoding passed |

All retained files rendered without obvious file corruption. The character remained notably stable across the retained scenes: red hair, green eyes, red scarf, dark jacket, and crescent motif were consistently recognizable. Story and indoor compositions were clean, and the sketchbook action was retained. There is still semantic drift: hair and facial details vary, and the third full-body repetition added an extra floating crescent accessory. A technically valid PNG therefore remains weaker evidence than a reviewed production frame.

The operational result is promising for the Animation Workflow, but limited. The retained set does not support claims about outdoor or dynamic scenes, two-character interaction, typography, photorealism, editing, or superiority to another candidate. Those dimensions require a separately authorized benchmark rather than extrapolation from this early stop.

The managed `qwen-image` mode was verified as the only active model mode. Its acceptance sequence passed local health, the Cloudflare Access-protected public endpoint boundary, and a real image generation. Qwen-specific `true_cfg_scale` and negative-prompt behavior are explicit configuration rather than hard-coded benchmark assumptions.
