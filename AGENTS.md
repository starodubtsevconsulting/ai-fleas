...
flowchart LR
  ...
  Hermes --> HermesResult["Initialize workflow role roster + group"]
  Scope --> GPT
  Scope --> Hermes
```
...
| `Initialize client-a-dev for Hermes` | Select fictional profile `client-a`, workflow `dev`, platform `hermes`, and logical project `client-a-dev` → resolve profile command `hermes-app` → load its config and `hermes-app/hermes-app.command.sh` → invoke `initialize` through `platforms/hermes`. | Initialize the Hermes role profiles and workflow group declared by that adapter. |
| `Re-init client-a-dev for Hermes` | Resolve the same exact fictional profile, workflow, project, command binding, and platform scope → invoke `hermes-app reinitialize --confirm-reinitialize`. | Delete the exact existing Hermes role profiles and group, then create a fresh complete generation; prior conversations and memory for those profiles are removed. |
| `Check whether GPT App has an update` | Resolve physical product name to command `chatgpt` under the `install` group → invoke read-only `check-update` through the host application's trusted stable update channel. | Report installed/latest stable evidence and recommend a separate explicit upgrade, or return `GPT_APP_UPDATE_CHECK_UNAVAILABLE` without mutation. |
| `Check whether Hermes App has an update` | Resolve physical product name to command `hermes` under the `install` group → invoke read-only `check-update`. | Compare installed, latest stable, and supported pinned releases; recommend but never automatically perform an upgrade. |
| `Install GPT` | Resolve human alias `GPT` to command `chatgpt` under the `install` command group → load `install/chatgpt/chatgpt.command.md` → invoke its `install` action. Do not route to `install/codex`, which installs only the Codex CLI. | Install and verify the desktop application, or return `GPT_APP_INSTALL_UNAVAILABLE` without mutation when the host cannot perform the trusted installation. |
| `Update Hermes` | Resolve human alias `Hermes` to command `hermes` under the `install` command group → invoke its read-only `update`/`check-update` action. | Report stable update evidence and recommend a separately authorized `upgrade`; do not update automatically. |
| `Upgrade <application>` | Resolve the exact child command under the `install` command group; do not route to the application's agent lifecycle command. | Apply and verify the reviewed stable upgrade only after explicit authorization. |
| `Uninstall <application>` | Resolve the exact canonical installation target, verify adapter ownership, and require explicit confirmation. | Remove only physical adapter-owned application artifacts; preserve agents, profiles, projects, groups, conversations, repositories, credentials, and other user data unless separately authorized. |
| `Delete the client-a-dev GPT group` | Resolve fictional profile `client-a`, workflow `dev`, platform `gpt-app`, and complete logical project `client-a-dev` → load its recorded task and sidebar-section bindings → invoke `gpt-app delete-workflow`. Treat “group” as human wording for the logical workflow scope, not as a saved Codex project. | Recoverably archive every exact bound agent task and delete the exact custom sidebar section while preserving the saved Codex project, checkout, repository, and work target. |
| `Delete example-dev` without a verified binding | A visible name alone cannot identify a logical project, sidebar section, task roster, or platform safely. | Ask for the missing exact scope and perform zero mutation. |
