...
| `Check whether GPT App has an update` | Resolve platform `gpt-app` → load `gpt-app/gpt-app.command.md` → invoke read-only `check-update` through the host application's trusted stable update channel. | Report installed/latest stable evidence and recommend a separate explicit upgrade, or return `GPT_APP_UPDATE_CHECK_UNAVAILABLE` without mutation. |
| `Check whether Hermes App has an update` | Resolve platform `hermes` → load `hermes-app/hermes-app.command.sh` → invoke read-only `check-update`. | Compare installed, latest stable, and supported pinned releases; recommend but never automatically perform an upgrade. |
| `Install GPT` | Resolve human alias `GPT` to canonical installation target `gpt-app` → load `install/install.command.md` → invoke its reviewed target adapter. Do not route to `install/codex`, which installs only the Codex CLI. | Install and verify the desktop application, or return `GPT_APP_INSTALL_UNAVAILABLE` without mutation when the host cannot perform the trusted installation. |
| `Update Hermes` | Resolve human alias `Hermes` to canonical installation target `hermes-app` → invoke the generic `install` command's read-only `update`/`check-update` action. | Report stable update evidence and recommend a separately authorized `upgrade`; do not update automatically. |
| `Upgrade <application>` | Resolve the exact canonical installation target and use `install`; do not route to the application's agent lifecycle command. | Apply and verify the reviewed stable upgrade only after explicit authorization. |
